...
import { buildMetricsHealthCheck } from '~/server/admin/health-checks';
...
export async function get({ request, clientAddress }: { request: Request; clientAddress?: string }) {
  const auth = await authorizeAdminRequest(request, {
    scope: 'admin_health_metrics_api',
    clientAddress,
  ...
  const result = await buildMetricsHealthCheck(auth.method, { strictMode, city });
