...
| `npm run build:semantic`      | Regenerate semantic core, clusters, forecast, rollout policy |
| `npm run build`               | Build your production site to `./dist/`                      |
| `npm run preview`             | Preview your build locally, before deploying                 |
| `npm run check`               | Check your project for errors                                |
| `npm run check:semantic`      | Validate semantic artifacts + deterministic sync             |
| `npm run check:rollout-gates` | Validate factory rollout constraints in generated pages      |
| `npm run fix`                 | Run Eslint and format codes with Prettier                    |
| `npm run astro ...`           | Run CLI commands like `astro add`, `astro preview`           |

<br>

### SEO Factory Workflow (Required)

- Generated pages are versioned artifacts: `data/generated-pages.json` must be committed.
- Semantic artifacts are versioned: `semantic-observed-raw.json`, `semantic-core.json`, `semantic-clusters.json`, `semantic-forecast.json`, `factory-rollout-policy.json`.
- Any semantic/input changes require `npm run build:semantic` before commit.
- Any change to geo/service templates, generation rules, or URL mapping requires `npm run build:data` before commit.
- CI gates `npm run check:semantic`, `npm run check:generated-pages-sync`, and `npm run check:rollout-gates` must pass.
- Architecture invariants are defined in [`ARCHITECTURE.md`](./ARCHITECTURE.md) and are mandatory for URL, sitemap/robots, district policy, and env contracts.
...
Production edge/runtime for this repo is **Netlify**.
API routes under `/api/*` must run as platform functions (not static files).

#### Static build (manual)

You can create an optimized static build with:
...
Current Docker/Nginx setup serves static `dist` only and is suitable for static preview.
It is not a production runtime for `/api/*`.
